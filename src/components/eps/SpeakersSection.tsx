...
import speakerMagdalena from "@/assets/eps/speaker-magdalena-centered.png";
import speakerJana from "@/assets/eps/speaker-jana.png";
import speakerToby from "@/assets/eps/speaker-toby.png";
import logoSsip from "@/assets/eps/logo-ssip-biotesc.png";
import logoInnovian from "@/assets/eps/logo-innovian.png";
import logoMassbalance from "@/assets/eps/logo-massbalance.png";
...
const SpeakersSection = () => {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 eps-bg-gradient-dark" style={{ opacity: 0.88 }} />
      <div className="relative z-10 eps-section-padding max-w-6xl mx-auto">
        <AnimatedSection className="text-center mb-16">
          <p className="eps-label-caps mb-4">Faculty</p>
          <h2 className="eps-heading-section">Speakers</h2>
        </AnimatedSection>
          ...
          {speakers.map((speaker, i) => (
            <AnimatedSection key={speaker.name} delay={i * 0.08}>
              <motion.div
                ...
                className="eps-glass-card eps-glow-border rounded-lg p-8 cursor-pointer transition-all duration-500 h-full"
                whileHover={{ y: -2 }}
              >
                {speaker.photo ? (
                  <img
                    ...
                    style={{ border: "1px solid hsl(168 100% 42% / 0.2)" }}
                  />
                ) : (
                  <div
                    className="w-20 h-20 rounded-full mb-6 flex items-center justify-center text-xl font-light"
                    style={{
                      background: "linear-gradient(135deg, hsl(168 100% 42% / 0.15), hsl(215 50% 55% / 0.1))",
                ...
                <p className="text-sm mb-1 min-h-[2.5rem]" style={{ color: "hsl(168 100% 42% / 0.8)" }}>
                  {speaker.title}
                </p>
                {speaker.orgLogo ? (
                ...
                <AnimatePresence>
                  {expanded === i && (
                    <motion.p
                      ...
                      className="eps-body-regular text-sm overflow-hidden"
                    >
                      {speaker.bio}
                    </motion.p>
                  )}
