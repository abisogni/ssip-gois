...
const FocusAreasSection = () => (
  <section className="relative overflow-hidden">
    <BiotechFiberBg />
    <div className="absolute inset-0 eps-bg-gradient-section" style={{ opacity: 0.88 }} />
    <div className="relative z-10 eps-section-padding max-w-6xl mx-auto">
      <AnimatedSection className="text-center mb-16">
        <p className="eps-label-caps mb-4">Focus Areas</p>
        <h2 className="eps-heading-section">Strategic Domains for Industrial Application</h2>
      </AnimatedSection>
        ...
        {areas.map((area, i) => (
          <AnimatedSection key={area.title} delay={i * 0.08}>
            <motion.div
              ...
              className="eps-glass-card eps-glow-border rounded-lg p-8 h-full transition-all duration-500"
            >
              <area.icon className="w-8 h-8 mb-5" style={{ color: "hsl(168 100% 42%)" }} />
              <h3 className="text-lg font-medium mb-3" style={{ color: "hsl(var(--foreground))" }}>{area.title}</h3>
              <p className="eps-body-regular text-sm">{area.desc}</p>
            </motion.div>
          </AnimatedSection>
        ))}
      </div>
