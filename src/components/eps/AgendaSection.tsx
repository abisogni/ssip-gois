...
const AgendaSection = () => {
  ...
  return (
    <section id="agenda" className="relative overflow-hidden">
      <DataGridBg />
      <div className="absolute inset-0 eps-bg-gradient-section" style={{ opacity: 0.9 }} />
      <div className="relative z-10 eps-section-padding max-w-4xl mx-auto">
        <AnimatedSection className="text-center mb-16">
          <p className="eps-label-caps mb-4">Programme</p>
          <h2 className="eps-heading-section mb-4">Agenda</h2>
          <p className="eps-body-regular text-sm" style={{ color: "hsl(var(--muted-foreground) / 0.7)" }}>
        ...
        <div className="space-y-0">
          {agenda.map((item, i) => (
            <AnimatedSection key={i} delay={i * 0.04}>
              <div
                className="group relative border-l pl-8 py-5 cursor-default transition-all duration-500"
              ...
              >
                <div
                  ...
                  style={{
                    backgroundColor: hoveredIndex === i ? "hsl(168 100% 42%)" : "hsl(var(--muted-foreground) / 0.3)",
                    boxShadow: hoveredIndex === i ? "0 0 12px hsl(168 100% 42% / 0.5)" : "none",
                  }}
                />

                <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-6">
                  <span className="text-sm font-medium shrink-0" style={{ color: "hsl(168 100% 42% / 0.7)" }}>
                    {item.time}
                  </span>
                  <div>
                    ...
                    {(item as any).speaker && (
                      <p className="text-xs font-light tracking-wide mb-1" style={{ color: "hsl(168 100% 42% / 0.6)" }}>
                        {(item as any).speaker}
                      </p>
                    )}
                    <AnimatePresence>
